---
title: "Repaso"
author: "Lucas Melfi"
format: html
editor: visual
---

## Quarto

Quarto enables you to weave together content and executable code into a finished document. To learn more about Quarto see <https://quarto.org>.

## Running Code

When you click the **Render** button a document will be generated that includes both content and the output of embedded code. You can embed code like this:

```{r}
1 + 1
```

You can add options to executable code like this

```{r}
#| echo: false
2 * 2
```

The `echo: false` option disables the printing of code (only output is displayed).

# ===================================================================

# BLOQUE 1: FILTROS Y AGREGACIONES BÁSICAS (Calentamiento)

# ===================================================================

# Ejercicio 1.1: Promedio de gasto en alimentos

# Calcula el promedio de gasto en alimentos de todos los hogares.

# Pista: usa summarise() y mean()

```{r}
hogares %>% 
  summarise(media_alimentos = mean(alimentos, na.rm=TRUE))

```

# Ejercicio 1.2: Hogares con alto gasto en salud

# Filtra los hogares que gastan más de 5000 en salud.

# ¿Cuántos hogares son?

# Pista: usa filter() y nrow() o count()

```{r}
hogares %>% 
  filter(salud > 500) %>%
  count()

hogares %>% 
  filter(salud > 500) %>%
  nrow()

```

# Ejercicio 1.3: Distribución por sexo

# Cuenta cuántas personas hay de cada sexo (1=Hombre, 2=Mujer).

```{r}
personas %>%
  count(sexo)

personas %>%
  group_by(sexo) %>% 
  summarise(n= n ())
```

# BLOQUE 2: JOINS SIMPLES (Relacionando tablas)

# Ejercicio 2.1: Hogares con sus viviendas

# Une la tabla de hogares con viviendas para ver el tipo de vivienda

# de cada hogar.

```{r}

hog_viv <- hogares  %>%
  left_join(viviendas, by="folioviv")

#Seleccionando dos columnas solamente
hog_viv2 <- hogares %>%
  left_join(viviendas %>% select(folioviv, tipo_viv), by="folioviv")

# ❌ Incorrecto
select(columna_que_quiero)

# ✅ Correcto
select(columna_del_by, columna_que_quiero)

```

# Ejercicio 2.2: Personas en hogares de clase específica

# Obtén todas las personas que viven en hogares de clase_hog = 1.

```{r}
ch1 <- left_join(personas, hogares, by=c("folioviv", "foliohog")) %>% filter(clase_hog==1)

ch2  <- personas %>%
  inner_join(hogares %>% 
               filter(clase_hog == 1), by=c("folioviv", "foliohog"))
ch3 <- personas %>%
  semi_join(
    hogares %>% filter(clase_hog == 1),
    by = c("folioviv", "foliohog")
  )

```

# Ejercicio 2.3: Hogares sin vivienda registrada

# Encuentra hogares que NO tienen registro en la tabla viviendas.

```{r}
sh <- anti_join(hogares, viviendas, by="folioviv")

```

# ===================================================================

# BLOQUE 3: AGREGACIONES CON GROUP_BY (Nivel medio)

# ===================================================================

# Ejercicio 3.1: Gasto promedio por ubicación geográfica

# Calcula el gasto monetario promedio (gasto_mon) por ubica_geo.

# Ordena de mayor a menor gasto.

```{r}
gasto_geo <- hogares %>%
  group_by(ubica_geo) %>%
  summarise(media_geo = mean(gasto_mon, na.rm = TRUE)) %>%
  arrange(media_geo)
```

# Ejercicio 3.4: Porcentaje de analfabetismo por ubicación

# Calcula qué porcentaje de personas NO sabe leer/escribir (alfabetism=2)

# en cada ubica_geo.

```{r}
per_geo <- personas %>%
  inner_join(hogares, by=c("folioviv", "foliohog"))%>% 
  group_by(ubica_geo) %>%
  summarise(total = n(), 
            analfabetos = sum(alfabetism==2), 
            porcentaje = (analfabetos / total)*100)

per_geo2 <-  personas %>% 
  inner_join(hogares, by=c("folioviv", "foliohog")) %>%
  group_by(ubica_geo) %>%
  summarise(porcentaje = mean(alfabetism==2)*100)
```

# ===================================================================

# BLOQUE 4: FILTROS COMPLEJOS CON ALL/ANY (Desafiante)

# ===================================================================

# Ejercicio 4.1: Hogares donde TODOS tienen educación

# Encuentra hogares donde todas las personas asisten a la escuela

# (asis_esc = 1) o son mayores de 18 años.

```{r}
fil4 <- hogares %>%
  semi_join(
    personas %>%
      group_by(folioviv, foliohog) %>%
      summarise(todos_cumplen = all(asis_esc == 1 | edad > 18, na.rm = TRUE), .groups = "drop") %>%
      filter(todos_cumplen),
    by = c("folioviv", "foliohog")
  )
#menos costo computacional, trabajas sobre una planilla y hace el join 

fil4_1 <- personas %>%
  group_by(folioviv, foliohog) %>%
  summarise(todos_cumplen = all(asis_esc == 1 | edad > 18, na.rm = TRUE), .groups = "drop") %>%
  filter(todos_cumplen) %>%
  inner_join(hogares, by = c("folioviv", "foliohog"))



```

# Ejercicio 4.2: Hogares con AL MENOS un trabajador

# Identifica hogares donde al menos una persona tiene trabajo registrado

# en la tabla trabajos

```{r}
trabajos %>% distinct(folioviv, foliohog) 
trabajos %>% distinct(folioviv, foliohog) %>% nrow()

```

# Ejercicio 4.3: Hogares donde NADIE habla lengua indígena

# Encuentra hogares donde ninguna persona habla lengua indígena

# (hablaind = 2 significa NO habla).

```{r}
personas %>%
  group_by(folioviv, foliohog) %>%
  summarise(
    nadie_habla_indigena = all(hablaind == 2, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  filter(nadie_habla_indigena)

```

# BLOQUE 4.5: EJERCICIOS ADICIONALES - PRÁCTICA INTENSIVA
# (Mismo nivel que Bloque 4, más variedad)
# ===================================================================

# Ejercicio 4.5.1: Hogares sin menores de edad
# Encuentra hogares donde TODAS las personas son mayores de 18 años.

```{r}
personas %>%
  group_by(folioviv, foliohog)%>%
  summarise(
    hog_18 = all(edad >= 18, na.rm=TRUE), 
    .groups = "drop")%>%
  filter(hog_18)%>%
  nrow()

```
# Ejercicio 4.5.2: Hogares con diversidad de edades
# Encuentra hogares que tengan AL MENOS una persona menor de 18 Y
# AL MENOS una persona mayor de 60.

```{r}
#mezclados
personas %>%
  group_by(folioviv, foliohog) %>%
  summarise(
    diversidad = any(edad < 18, na.rm = TRUE) & any(edad > 60, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  filter(diversidad)

#separados
personas %>%
  group_by(folioviv, foliohog) %>%
  summarise(
    tiene_menores = any(edad < 18, na.rm = TRUE),    # ¿Hay menores?
    tiene_mayores = any(edad > 60, na.rm = TRUE),    # ¿Hay mayores?
    .groups = "drop"
  ) %>%
  filter(tiene_menores & tiene_mayores)  # AMBAS condiciones

```
# Ejercicio 4.5.3: Hogares completamente alfabetizados con niños
# Encuentra hogares donde:
# - TODAS las personas son alfabetizadas (alfabetism == 1)
# - Y tienen AL MENOS un menor de 12 años

```{r}
personas %>%
  group_by(folioviv, foliohog) %>%
  summarise(
    alfab_hijos= all(alfabetism == 1, na.rm = TRUE) & any(edad>=1, na.rm = TRUE), 
            .groups = "drop")%>%
  filter(alfab_hijos)%>%
  nrow()

```

# Ejercicio 4.5.4: Hogares sin ingresos registrados
# Encuentra hogares que NO tienen ningún registro en la tabla ingresos.

```{r}
hogares %>% anti_join(ingresos, by=c("folioviv", "foliohog"))

# devuelve filas de la tabla izquierda que NO tienen coincidencia en la tabla derecha.

```

# Ejercicio 4.5.5: Hogares donde nadie trabaja
# Encuentra hogares que NO tienen ninguna persona registrada en trabajos.
# ¿Cuántos hogares son? ¿Cuál es su gasto promedio?

```{r}
hogares %>% 
  anti_join(trabajos, by=c("folioviv", "foliohog"))%>%
  summarise(cant_hog = n(), 
            gasto_medio = mean(gasto_mon))
  

```
# Ejercicio 4.5.6: Hogares multigeneracionales
# Encuentra hogares con AL MENOS 3 generaciones:
# - Al menos una persona < 18 años (niños)
# - Al menos una persona entre 18-59 años (adultos)
# - Al menos una persona >= 60 años (adultos mayores)

```{r}
personas %>%
  group_by(folioviv, foliohog) %>%
  summarise(multigen = any(edad<18, na.rm = TRUE) & any(edad ==18 & edad<=59, na.rm = TRUE) & any(edad>=60, na.rm = TRUE), 
            .groups = "drop")%>%
  filter(multigen)%>%
  nrow()


#alternativa
personas %>%
  group_by(folioviv, foliohog) %>%
  summarise(
    tiene_menor  = any(!is.na(edad) & edad < 18),
    tiene_adulto = any(!is.na(edad) & edad >= 18 & edad <= 59),
    tiene_mayor  = any(!is.na(edad) & edad >= 60),
    .groups = "drop"
  ) %>%
  filter(tiene_menor & tiene_adulto & tiene_mayor) %>%
  nrow()

```
# Ejercicio 4.5.7: Hogares homogéneos por sexo
# Encuentra hogares donde TODAS las personas son del mismo sexo.
```{r}

#alternativa para ver cuandos de cada uno
personas %>%
  group_by(folioviv, foliohog) %>%
  summarise(sex_m = all(sexo == 1, na.rm = TRUE),
            sex_f = all(sexo == 2, na.rm = TRUE),
            .groups = "drop")%>%
  filter(sex_m | sex_f) %>% 
  nrow() #number of rows: devuelve cuantas filas hay

  personas %>%
    group_by(folioviv, foliohog) %>%
    summarise(
      mismo_sexo = n_distinct(sexo, na.rm = TRUE) == 1,
      .groups = "drop"
    ) %>%
    filter(mismo_sexo) %>%
    nrow()
#No le pide que tome las filas con sexo 1, sino le dice que cuente los hogares cuyas filas (personas) son todas de un mismo valor

```
```{r}

n_distinct(folioviv, foliohog)

#Es una función de dplyr que cuenta cuántos valores distintos hay en una variable.
#En este caso cuenta combinaciones únicas de vivienda-hogar.

```

#es casi equivalente a:
```{r}
trabajos %>%
  distinct(folioviv, foliohog) %>%
  nrow()
```

#Es decir:

distinct() crea la tabla de hogares únicos.
nrow() cuenta cuántas filas tiene.

n_distinct() hace ambas cosas en un solo paso.

Por eso suele verse mucho dentro de summarise().

#Hay funciones que trabajan dentro de los grupos: 
(n(), n_distinct(), mean(), sum(), any(), all())  

#Hay funciones que actúan sobre el resultado final: 
(filter(), nrow(), arrange(), etc.).

# Ejercicio 4.5.8: Hogares con alta asistencia escolar
# Encuentra hogares donde TODAS las personas menores de 18 años
# asisten a la escuela (asis_esc == 1).

```{r}

```

